...
func TestMarshalRawPacket(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil-data", &ipod.RawPacket{ipod.NewLingoCmdID(0x01, 0x02), nil},
			[]byte{0x55, 0x02, 0x01, 0x02, 256 - 0x05}, false},
		{"empty-data", &ipod.RawPacket{ipod.NewLingoCmdID(0x01, 0x02), []byte{}},
			[]byte{0x55, 0x02, 0x01, 0x02, 256 - 0x05}, false},
		{"small-data", &ipod.RawPacket{ipod.NewLingoCmdID(0x01, 0x02), []byte{0xfd}},
			[]byte{0x55, 0x03, 0x01, 0x02, 0xfd, 0xfd}, false},
		{"large-data", &ipod.RawPacket{ipod.NewLingoCmdID(0x01, 0x02), largeData},
			append([]byte{0x55, 0x00, 0x01, 0x01, 0x1, 0x02}, append(largeData, 0xe9)...), false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ipod.MarshalRawPacket(tt.pkt)
...
func BenchmarkUnmarshalRawPacket(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ipod.UnmarshalRawPacket(shortFrameB[2:])
	}
}
